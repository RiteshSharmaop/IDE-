...
const VariableProximity = forwardRef((props, ref) => {
  ...
  useAnimationFrame(() => {
    ...
    letterRefs.current.forEach((el, i) => {
      ...
      const distance = calculateDistance(x, y, letterX, letterY);
      ...
      const newSettings = parsedSettings.map(({ axis, fromValue, toValue }) => {
        return `'${axis}' ${fromValue + (toValue - fromValue) * falloffValue}`;
      }).join(', ');

      interpolatedSettingsRef.current[i] = newSettings;
  ...
  return (
    ...
    >
      {lines.map((line, lineIdx) => (
        <div key={lineIdx} className="inline-block w-full text-center leading-snug">
